...
var { ElInput, ElIcon,ElSelect, ElOption,ElRadio,ElRadioButton,ElRadioGroup,ElCheckboxGroup,ElCheckbox,ElCheckboxButton,ElCascader} = require("element-plus");
import {Warning} from '@element-plus/icons-vue'



import {CreateNodeFactory} from './core/factory/node';
...
var FcIcon = vue.defineComponent({
	name: 'FcIcon',
	inheritAttrs: false,
	props:{
		is:{
	...
	render: function render() {
		return vue.createVNode(
			...
			[vue.createVNode(vue.resolveComponent("Warning"))]
		);
	}
});
...
var TreeSelect = vue.defineComponent({
	...
	render: function render() {
		...
		return vue.createVNode(
			vue.resolveComponent("ElTreeSelect"),
			vue.mergeProps(
				this.$attrs,
				{
					modelValue: this.value,
...
function FormCreate(vm) {
	...
	extend(this, {
		...
		makeInfo: function makeInfo(rule, uni) {
			...
			return titleFn();
				 ...
				 // return _this4.$r(mergeProps(),children)
	...
	extend(this, {
		install: () => {
			[Radio, Select,Group,Checkbox,Cascader,TreeSelect,FcIcon].forEach(function (component) {
