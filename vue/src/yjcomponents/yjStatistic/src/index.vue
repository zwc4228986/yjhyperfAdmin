	...
	<el-card shadow="never">
		<template #header>
			<div class="card-header">
				<span>{{ title }}</span>
				<el-tag class="ml-2" v-if="tag" type="success">{{tag}}</el-tag>
			</div>
		</template>
		...
		<div class="sc-statistic">
			...
			<div v-if="description || $slots.default" class="sc-statistic-description">
				...
				<div>
					<span v-if="prefix" class="sc-statistic-content-prefix">{{ prefix }}</span>
					<span class="sc-statistic-content-value">{{ description_value }}</span>
					<span v-if="suffix" class="sc-statistic-content-suffix">{{ suffix }}</span>
				</div>
			</div>
		</div>
...
export default {
	props: {
		...
		description_value: { type: String, default: "" },
...
.sc-statistic-description {margin-top: 10px;padding-top: 10px;color: #999;display:flex;justify-content:space-between;border-top: 1px solid var(--el-card-border-color);}
.card-header{
	display: flex;
