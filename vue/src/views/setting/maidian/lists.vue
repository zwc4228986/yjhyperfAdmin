...
<script>
	...
	export default {
		...
		data() {
			return {
				formParams: {},
				apiObj: this.$HTTP().url("/admin/maidian/lists"),
				tableData: [],
				selection: [],
			};
		},
		...
		methods: {
			...
			onEdit({id}){
				this.$modalForm(
					this.$HTTP().params({id}).post("/admin/maidian/create")
				).then((res) => {
			...
			onAdd(){
				this.$modalForm(
					this.$HTTP().post("/admin/maidian/create")
				).then((res) => {
					 this.$refs.yjTable.upData(this.formParams)
				});
			},
			...
			handleDelete({id}) {
				...
				this.$confirm("此操作会将数据物理删除？", "提示", {
					...
					this.$HTTP().params({id}).post('admin/maidian/delete').then((res) => {
