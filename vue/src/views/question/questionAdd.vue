  ...
  >
    <el-container>
      <el-main style="padding: 0 20px 20px 20px">
        <!--				<el-tabs v-model="form.types" class="demo-tabs" @tab-click="handleClick">-->
        <!--					<el-tab-pane label="得分题" name="score"></el-tab-pane>-->
        ...
        >
          ...
          <el-form-item prop="column" style="width: 100%">
            ...
            >
              ...
              <!-- <el-table-column prop="types"  width="300" label="得分类型">
								<template #default="scope">
									<el-input type="textarea" v-model="scope.row.types" placeholder="请输入名称"></el-input>
								</template>
							</el-table-column> -->
              <el-table-column prop="fixed" label="题目分数" width="150">
                <template #default="scope">
                  <el-input-number
...
export default {
  ...
  methods: {
    async show({ id }) {
      ...
      if (id) {
        this.$HTTP()
          ...
          .post("admin/question/question/detail")
          .then((res) => {
            this.form = res;
          });
      } else {
    ...
    addDefaultColumn() {
      if (this.form.question_detail.length == 0) {
        this.form.question_detail.push(this.addTemplate);
      }
    },
    getInfo() {
      this.$HTTP()
    ...
    onSubmit() {
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          ...
          let res = await this.$HTTP().params(this.form).post(this.getAction());
          this.$emit("success");
