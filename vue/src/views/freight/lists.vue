<template>
  <el-container>
    ...
    <el-main class="nopadding">
      <yjTable2 ref="yjTable" :params="formParams" :formConfig="formConfig">
        <template v-slot:op>
          <el-table-column
            label="操作"
            fixed="right"
  ...
  <freightAdd ref="freightAdd"></freightAdd>
</template>
<script>
import freightAdd from "./add";
...
export default {
  ...
  methods: {
    onReport(data) {
      this.$refs.freightAdd.show(data);
    },
    onEdit({ id }) {
      this.$refs.freightAdd.show(id);
    },
    onAdd() {
      this.$refs.freightAdd.show(0);
    },
    success(res) {
      console.log(res);
      this.$HTTP()
