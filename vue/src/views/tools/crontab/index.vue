...
<template>
    ...
    <el-row :gutter="15">
      ...
      >
        <el-card class="task task-item" shadow="hover">
          ...
          <div class="bottom">
            <div class="state">
              <el-tag v-if="item.status == '1'" size="small">进行中</el-tag>
              <el-tag v-if="item.status == '-1'" size="small">异常</el-tag>
              <el-tag v-if="item.status == '0'" size="small" type="info">
                停用
              </el-tag>
            </div>
            <div class="handler">
              ...
              >
                <template #reference>
                  <el-button type="primary" circle>
                    <i-pause theme="outline" size="14" fill="#fff" />
                  </el-button>
                </template>
              </el-popconfirm>
              <el-popconfirm
              ...
              >
                <template #reference>
                  <!-- <el-button type="warning" icon="el-icon-videopause" circle></el-button> -->
                  <el-button type="primary" circle>
                    <i-pause theme="outline" size="14" fill="#fff" />
                  </el-button>
                </template>
              ...
              <el-dropdown trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item @click="del(item)" divided>
                      删除
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </template>
  ...
  <el-drawer
    title="计划任务日志"
    v-model="draw_crontab_show"
    :size="600"
    direction="rtl"
    destroy-on-close
  >
    <logs :crontab_id="crontab_id"></logs>
  </el-drawer>
</template>
...
export default {
  ...
  methods: {
    async lists() {
      const data = await this.$HTTP().post("admin/crontab/lists");
      this.list = data.data;
    },
    add() {
