<template>
  ...
  <report ref="report"></report>
	<el-drawer title="" v-model="drawer" size="50%"  @closed="$emit('closed')">
  ...
  methods: {
    _init() {
      if (this.formConfig.tabs) {
        this.formParams[this.formConfig.tabs.prop] = "all";
      }
    },
    onReport(data) {
    ...
    onAudit(e){
      // this.drawer = true;
      window.location.href = "http://150.158.155.57:8003/#/pages/test/test?task_receive_id="+e.id+"&random="+Math.random()
