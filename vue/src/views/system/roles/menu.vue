              ...
              <block v-for="(vv, childIndex) in item.children" :key="vv.id">
                <el-row :gutter="24">
                  ...
                  <el-col :span="20">
                    <el-row
                      :gutter="24"
                      v-for="(vvv, threeIndex) in vv.children"
                      :key="vvv.id"
                    >
                      <el-col :span="4">
                        <el-checkbox
                      ...
                      <el-divider
                        v-if="threeIndex !== vv.children.length - 1"
                      ></el-divider>
                    </el-row>
                  </el-col>
                </el-row>
...
export default {
  ...
  methods: {
    ...
    getMenuLists(rolesData) {
      http()
        .params({ role_id: this.rolesData.id })
        .post("admin/menu/roles")
        .then((res) => {
    ...
    onSubmit() {
      ...
      this.$HTTP()
        .params({ roles_id: this.rolesData.id, menus_id: checkedMenusId })
        .post("admin/system/roles/set-menus")
        .then((res) => {
    ...
    getCheckedMenusId(menuLists) {
      ...
      this.collect(menuLists).every((res) => {
        if (res.children) {
          checkedMenusId = checkedMenusId.concat(
            this.getCheckedMenusId(res.children).all()
          );
        }
        if (res.checked == true) {
