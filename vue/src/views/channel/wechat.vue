...
<template>
	<el-container>
		<el-header>
			{{ menuSonIndex }}
			<el-alert type="warning" title="配置微信公众号菜单，点击确认，保存菜单并发布至微信公众号" :closable="false"
					  show-icon/>
		</el-header>
				...
				<div class="mt-4 lg:mt-0 max-w-[400px]" v-show="attrIndex === menuIndex && menuSonIndex == 0">
					<yjCloseWrap @close="handleDelMenu(attrIndex, 0)">
						...
						<el-form :model="form" label-width="120px">
							...
							<el-form-item label="网址" prop="url" v-if="menuData.type == 'miniprogram'">
								<el-input v-model="menuData.url" value="http://mp.weixin.qq.com"/>
							</el-form-item>
				...
				<template v-for="(menuSonData, attrSonIndex) in menuData.sub_button" :key="attrSonIndex">
					<yjCloseWrap @close="handleDelMenu(attrIndex, attrSonIndex + 1)">
						<div class="mt-4 lg:mt-0 max-w-[400px]"
							 v-show="attrIndex === menuIndex && (attrSonIndex + 1) == menuSonIndex">
