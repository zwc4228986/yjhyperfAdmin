		...
		<ul ref="tags">
			...
			>
				<router-link :to="tag">
					...
					<el-icon :size="18" v-if="!tag.meta.affix" @click.prevent.stop='closeSelectedTag(tag)'><el-icon-close/></el-icon>
				</router-link>
			</li>
		</ul>
	</div>
	...
	<transition name="el-zoom-in-top">
		...
		>
			...
			<li @click="closeOtherTabs()">
				<i class="el-icon-folder-delete"></i>关闭其他标签
			</li>
			<hr />
			<li @click="screen()">
				<i class="el-icon-full-screen"></i>全屏当前标签
			</li>
			<li @click="openWindow()">
				<i class="el-icon-copy-document"></i>在新的窗口中打开
			</li>
		</ul>
	</transition>
</template>
...
export default {
	...
	async created() {
		var menu = await this.$Store.dispatch("getMenus");
