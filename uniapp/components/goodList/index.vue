				...
				<view @click="goDetail(item)" class='item acea-row row-between-wrapper' hover-class="none">
					...
					<view class='underline'>
						<view class='text'>
							...
							<view class='money font-color'><text class='num'>{{(item.price*10).toFixed(2)}}</text>小豆</view>
							<view class='vip-money acea-row row-middle'
								v-if="item.is_vip && item.vip_price && item.vip_price > 0">{{(item.vip_price*10).toFixed(2) || 0}}小豆
