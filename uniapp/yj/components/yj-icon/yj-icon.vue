...
<script>
	...
	const fontUrl = 'https://at.alicdn.com/t/font_3435583_23ytjg5mzud.ttf'
	...
	export default {
		...
		computed: {
			uClasses() {
				let classes = []
				console.log(this.customPrefix + '-' + this.name);
			...
			// 通过图标名，查找对应的图标
			icon() {
				console.log(icons,'icon-' + this.name)
				// 如果内置的图标中找不到对应的图标，就直接返回name值，因为用户可能传入的是unicode代码
				return icons['icon-' + this.name] || this.name
			}
		},
		methods: {
			clickHandler(e) {
