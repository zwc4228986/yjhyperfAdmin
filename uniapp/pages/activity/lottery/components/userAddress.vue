<template>
	<view class="aleart" v-if="aleartStatus">
		<image :src="`${IMAGE_URL}/static/images/poster-close.png`" class="close" @click="posterImageClose"></image>
		<view class="title">
			<image class="title-img" :src="`${IMAGE_URL}/static/images/address-aleart-header.png`" mode=""></image>
		</view>
								...
								editAddress({
									...
									setTimeout(function() {
										if (that.cartId) {
											...
											that.couponId = '';
											uni.navigateTo({
												url: '/pages/activity/lottery/grids/index?addressId=' +
													that.id ? that
													.id : res.data
													.id
											});
