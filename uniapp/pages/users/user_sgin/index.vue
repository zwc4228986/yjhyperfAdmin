			...
			<view class="wrap">
				<!-- <view class="vip-card-wrap">
					<view class="left">
						...
						<view class="title">
							<text class="title-top">会员权益卡</text>
							<text class="title-bottom">更多特权等你来</text>
						</view>
					</view>
				...
				<view class="wrap-item">
					...
					<!-- 	<view class="content">
						...
						<div v-else style="padding: 20rpx;display: flex;justify-content: center;">
							<yj-button
							  bg="#FF9640"
							  color="white"
				...
				<!-- <view class="wrap-item daily-task">
					...
					<view class="task-box">
						<view class="task-item" v-for="(item,index) in task_list_0" :key="index" >
							<view class="left">
								<view class="point is-select"></view>
								<view class="task-title">{{item.name}}</view>
								<view class="reward">+{{item.point}}小豆</view>
							</view>
							<view class="right">
								<button v-if="item.receive_status == 0" @click="goEvent(item)" class="btn">{{item.button_text}}</button>
								<button v-if="item.receive_status == 1"   @click="goReceive(item)" class="btn">去领取</button>
								<button v-if="item.receive_status == 2" disabled class="btn">已经领取</button>
							</view>
						</view>
					</view>
				</view>
				...
				<view class="wrap-item ohter-task">
					<view class="title">其他任务</view>
					
					<view class="task-box">
						<view class="task-item" v-for="(item,index) in task_list_1" :key="index" >
							<view class="left">
								<view class="point is-select"></view>
								<view class="task-title">{{item.name}}</view>
								<view class="reward">+{{item.point}}小豆</view>
							...
							<view class="right">
								<button class="btn">{{item.button_text}}</button>
							</view>
						</view>
...
<!-- 			<view class='wrapper'>
			...
			<view class='wrapper wrapper2'>
				...
				<view class='tip2'>据说连续签到第{{day}}天可获得超额小豆，一定要坚持签到哦~~~</view>
				<view class='list3'>
			...
			<view class='signTip acea-row row-center-wrapper' :class='active==true?"on":""'>
				...
				<view class='signTipCon'>
					...
					<view class='integral'>获得{{integral}}小豆</view>
...
import { sign } from 'crypto';
	...
	export default {
		...
		computed: {
				...
				task_list_0:function(){
					return this.task_lists.filter((res)=>{
						return res.type==0
					})
				},
				task_list_1:function(){
					return this.task_lists.filter((res)=>{
						return res.type==1
					})
				},
		...
		methods: {
			...
			goEvent(item){
				let {event,path} = JSON.parse(item.event)
...
<style scoped lang="scss">
	...
	.wrap {
		...
		.vip-card-wrap {
			...
			width: 716rpx;
			height: 132rpx;
			background-image: url('/static/images/users/user_sgin/vip-bg.png');
		...
		.wrap-item {
			...
			.sign-in-box {
				...
				.content {
					display: grid;
					grid-template-columns: 132rpx 132rpx 132rpx 132rpx;
					grid-template-rows: 152rpx 152rpx ;
					grid-row-gap: 22rpx;
					...
					.is-select {
						background-image: url('/static/images/users/user_sgin/sign-select-bg.png');
					}
				}
				...
				.bottom-btn {
					...
					.btn {
						...
						height: 66rpx;
						margin: 0 auto;
						background-image: url('/static/images/users/user_sgin/tomorrow-again-btn.png');
						background-size: 100% 100%;
						background-repeat: no-repeat;
					}
				}
		...
		.daily-task, .ohter-task {
			...
			.task-box {
				...
				// flex-direction: row;
				.task-item {
					padding: 24rpx 28rpx 22rpx 32rpx;
					...
					.right {
						.btn {
							background-color: rgb(245, 131, 37);
							height: 48rpx;
							width: 120rpx;
							color: #fff;
							border-radius: 24rpx;
