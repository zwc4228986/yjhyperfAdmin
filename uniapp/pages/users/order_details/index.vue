<template>
    ...
    <!-- 顶部导航 -->
    <!-- <u-navbar bgColor="transparent" :title="titleText" leftIconColor="#fff" titleStyle="color: #fff;"></u-navbar> -->
    <!-- <page-meta background-text-style="#000000"><navigation-bar background-color="#fcc62f" :title="titleText" /></page-meta> -->
        ...
        <view class="map acea-row row-between-wrapper" v-if="orderInfo.shipping_type == 2">
			...
			<view class="good-info">
				...
				<view class="good-info-bottom">
					<text class="total">共两件商品</text>
					<text class="actualMoney">实付款：13600小豆（免运费）</text>
				</view>
			</view>
...
<!-- 			<div class="goodCall" @click="goGoodCall" v-if='routineContact == 0'>
			...
			<!-- <view class='wrapper'>
				<view class='item acea-row row-between'>
					<view>商品总价：</view>
					<view class='conter'>
						{{(parseFloat(orderInfo.total_price)+parseFloat(orderInfo.vip_true_price))}}小豆</view>
				</view>
				<view class='item acea-row row-between' v-if="orderInfo.pay_postage > 0">
					<view>配送运费：</view>
					<view class='conter'>{{parseFloat(orderInfo.pay_postage)}}小豆</view>
				</view>
				<view v-if="orderInfo.vip_true_price > 0" class='item acea-row row-between'>
					<view>会员商品优惠：</view>
					<view class='conter'>-{{parseFloat(orderInfo.vip_true_price)}}小豆</view>
				</view>
				...
				<view class='item acea-row row-between' v-if='orderInfo.coupon_id'>
					<view>优惠券抵扣：</view>
					<view class='conter'>-{{parseFloat(orderInfo.coupon_price)}}小豆</view>
				</view>
				<view class='item acea-row row-between' v-if="orderInfo.use_integral > 0">
					<view>积分抵扣：</view>
					<view class='conter'>-{{parseFloat(orderInfo.deduction_price)}}小豆</view>
				</view>
				<view class='actualPay acea-row row-right' v-if="!orderInfo.help_info.help_status">实付款：<text
						class='money font-color'>{{parseFloat(orderInfo.pay_price)}}小豆</text></view>
				<view class='actualPay acea-row row-right' v-else>
					...
					总代付：<text class='money font-color'>{{parseFloat(orderInfo.pay_price)}}小豆</text>
				</view>
			</view> -->
...
<!-- 
				<view class="more" v-if="(invoice_func || invoiceData) && orderInfo.paid && !orderInfo.refund_status"
					...
					<button class="contact-btn">
						<text class="iconfont icon-kefu3" style="margin-right: 12rpx;"></text>
						联系客服
						</button>
...
<style scoped lang="scss">
	...
	.good-info {
		...
		.good-info-container {
			border-top: 1rpx solid rgba(#cccccc, 0.5);
