<template>
	<view class="chat-box" :style="colorStyle">
		...
		<view class="chat-scroll-box">
			<scroll-view scroll-y="true" style="height: 100%;" @scroll="scroll" :scroll-top="scrollTop" @scrolltoupper="scrollToTop">
				...
				<view id="box" class="chat" ref="chat">
					<view v-for="(item, index) in records" :key="index" :id="`msg-${item.id}`">
						...
						<!-- <view class="day-box" >1111{{item._add_time}}</view> -->
						<view class="chat-item" :class="{ 'right-box': item.sender_id == myUid }">
							<image class="avatar" :src="item.avatar" mode=""></image>
							<!-- 消息 -->
							<view class="msg-box" v-if="item.types=='text'" v-html="item.content"></view>
							<!-- 图片 -->
							
							<view class="img-box" v-if="item.types == 'image'">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.titleName =options.nickname;
		...
		onUnload() {
			...
			this.$store.commit('webSocket/clearPeopleUnReadNum',this.receiver_id);
			uni.$off()
		},
		onReady() {
		...
		methods: {
			initialListeners(){
				uni.$on('PRIVATE_MESSAGE_RECEIVED', (message) => {
				...
				getChatRecord({
					...
					.then(res => {
						...
						let selector = '';
						 if (res.data.length) {
							if (this.page == 1) {
									selector = `#msg-${res.data[res.data.length - 1].id}`;
							} else {
			...
			sendText() {
				...
				this.sendMsg(this.con, 'text');
				this.con = '';
			},
			// ws发送
			sendMsg(content, type) {
				var res = this.$yjSocket.send({
					data: {
						content,
						type,
						receiver_id: this.receiver_id
			...
			uploadImg() {
				...
				self.$util.uploadImageChange('upload/image', function(res) {
					if (res.status == 200) {
						self.sendMsg(res.data.url, 'image');
					}
				}, (res) => {
