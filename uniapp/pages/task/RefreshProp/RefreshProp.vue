	...
	<view>
		<form @submit="goPay" :style="colorStyle">
			<view class="payment-top acea-row row-column" style="justify-content: center;padding-left: 20rpx;" >
				<span class="name">剩余刷新次数</span>
				<view class="pic">
					<span class="pic-font">{{ userinfo.refresh_num || 0 }}</span>
				</view>
			</view>
			<view class="payment">
...
<script>
	...
	// 	getRechargeApi
	// } from '@/api/user.js';
	import {taskerPayRefresh} from '@/api/tasker.js'
	import {
		userInfo
	} from '@/api/user2.js';
	import {
	...
	export default {
		...
		data() {
			...
			return {
				...
				rechar_id:0,
				rechargeAttention:[
					'1.刷新后任务将在首页列表和任务大厅展示被刷新的任务会 置于普通任务顶层，但是处于置顶任务下方。',
					'2.刷新排序为后来居上，任务刷新后排在第一位，后面有人 刷新，会被挤到第二位、后面再有人刷新，会到第三位、第 四位.....如此循环。',
				]
			};
		},
		...
		methods: {
			getSettlesTaskRefresh(){
				getSettlesTaskRefresh().then(res=>{
					this.picList = res;
