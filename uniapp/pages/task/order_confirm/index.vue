		...
		<view class='order-submission'>
			...
			<view class='wrapper'>
				...
				<view class='item acea-row row-between-wrapper'>
					<view class="item-name">订单编码</view>
					<view class="item-value">{{taskDetail.id}} </view>
				</view>
				<view class='item acea-row row-between-wrapper' >
					<view class="item-name">任务数量</view>
					<view class="item-value">{{taskDetail.stock}}个</view>
				</view>
				<view class='item acea-row row-between-wrapper' >
					<view class="item-name">任务单价</view>
					<view class="item-value">{{taskDetail.one_price}}元</view>
				</view>
				<view class='item acea-row row-between-wrapper' >
					<view class="item-name">任务总价</view>
					<view class="item-value">{{taskDetail.price}}元</view>
				</view>
			</view>
			...
			<view class='footer acea-row row-between-wrapper'>
				<view>合计:
					<text class='font-color'>￥{{taskDetail.price || 0}}</text>
				</view>
...
<script>
	...
	import {taskerDetail,taskReceive,taskerPay} from '@/api/tasker.js'
	import {
		orderConfirm,
		...
		methods: {
			getDetail(){
				taskerDetail({task_id:this.task_id}).then(res=>{
					this.taskDetail = res;
					this.cartArr[2].title = '可用余额:' + res.user.now_money;
				})
			},
			// 不开发票
			invCancel() {
