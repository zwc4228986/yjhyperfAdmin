...
<script>
	import {getHomeVideo} from '@/api/config.js'
	export default {
		name: 'popupAd',
		data() {
			return {
		...
		mounted(){
			if(!this.$Cache.has('homeVideoShow')){
				getHomeVideo().then(res=>{
					console.log('home_video',res);
					this.url=res.home_video.full_path;
					this.desc=res.home_desc;
					this.isShow = true;
				})
			}
		...
		methods: {
			...
			handleClose() {
				if(this.is_show){
					this.$Cache.set('homeVideoShow',true)
				}
				this.isShow = false
			}
		}
