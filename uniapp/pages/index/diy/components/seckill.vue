<template>
	<view v-show="!isSortType" >
		<view class="spike-box"  :class="conStyle?'borderRadius20':''"  :style="{marginTop:mbConfig*2+'rpx'}">
			<view class="hd">
				<view class="left">
					<!-- <image :src="imgUrl" class="icon" v-if="imgUrl"></image> -->
					<image src="/static/images/skillicon.png" class="icon"></image>
					...
					<navigator class="spike-item" :style="'margin-right:'+ lrConfig*2 +'rpx;'" v-for="(item,index) in spikeList" :key="index" :url="'/pages/activity/goods_seckill_details/index?id='+item.id+'&time='+datatime+'&status=1'"
					 hover-class='none'>
						<view class="img-box">
							...
							<!-- <view v-if="discountShow" class="msg flex-aj-center" :style="'color:'+ themeColor +';border-color:'+ themeColor +';'">{{item.discountNum}}折起</view> -->
						</view>
						<view class="info">
							<view v-if="titleShow" class="name line1">{{item.title}}</view>
							<view class="price-box">
								<text v-if="seckillShow" class="tips" :style="'background-color:'+ themeColor +';'">抢</text>
								<text v-if="priceShow" class="price" :style="'color:'+themeColor+';'"><text>￥</text>{{item.price}}</text>
							</view>
						</view>
					</navigator>
				</scroll-view>
...
<style lang="scss">
	...
	.spike-box {
		...
		background-image: url('https://xmbshop.haishuju.vip/static/images/skillbg.png');
